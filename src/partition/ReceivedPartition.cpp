...
void ReceivedPartition::compute()
{
  ...
  _considerConnectivityOnFiltering = std::any_of(std::begin(_fromMappings), std::end(_fromMappings), [&](const auto &fromMapping) { return fromMapping->hasConstraint(mapping::Mapping::SCALEDCONSISTENT); }) or std::any_of(std::begin(_toMappings), std::end(_toMappings), [&](const auto &toMapping) { return toMapping->hasConstraint(mapping::Mapping::SCALEDCONSISTENT); });
  ...
  mesh::filterMesh(
      filteredMesh, *_mesh, [&](const mesh::Vertex &v) { return v.isTagged(); }, _considerConnectivityOnFiltering);
  ...
  if (_geometricFilter == ON_MASTER) { //filter on master and communicate reduced mesh then
    ...
    if (utils::MasterSlave::isSlave()) {
      ...
      for (int rankSlave = 1; rankSlave < utils::MasterSlave::getSize(); rankSlave++) {
        ...
        mesh::filterMesh(
            slaveMesh, *_mesh, [&slaveBB](const mesh::Vertex &v) { return slaveBB.contains(v); }, _considerConnectivityOnFiltering);
      ...
      mesh::Mesh filteredMesh("FilteredMesh", _dimensions, mesh::Mesh::MESH_ID_UNDEFINED);
      mesh::filterMesh(
          filteredMesh, *_mesh, [&](const mesh::Vertex &v) { return _bb.contains(v); }, _considerConnectivityOnFiltering);
